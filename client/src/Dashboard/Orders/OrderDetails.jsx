...
import { ALLORDERDETAILS, BASEURL, DELETEORDERDETAIL, EXPORTORDERDETAILS, FILTERORDERDETAILS } from "../../Api/Api";
function OrderDetails() {
    ...
    async function getOrders() {
        try {
            setLoading(true);
            let res = await AXIOS.get(`${BASEURL}/${ALLORDERDETAILS}?orderID=${orderID}&pageNumber=${activePage}&limitOfOrdersDetails=${ItemsPerPage}`);
    ...
    async function getSearchResults() {
        ...
        try {
            setLoading(true);
            let res = await AXIOS.get(`${BASEURL}/${FILTERORDERDETAILS}=${filterType}&orderID=${orderID}&value=${search}&pageNumber=${activePage}&limitOfOrders=${ItemsPerPage}`);
            setTotal(res.data.total);
            setOrders(res.data.data);
        }
        catch (err) {
    ...
    const handleExport = async () => {
        try {
            const response = await AXIOS.get(`${BASEURL}/${EXPORTORDERDETAILS}?pageNumber=${activePage}&limitOfOrdersDetails=${ItemsPerPage}`, {
                responseType: "blob",
            });
            ...
            console.error("Error exporting OrderDetails:", error.response?.data || error.message);
            alert("Failed to export OrderDetails. Please try again.");
        }
        finally {
    ...
    async function handleDelete(ID) {
        try {
            setLoading(true);
            let res = await AXIOS.delete(`${BASEURL}/${DELETEORDERDETAIL}/${ID}`);
            ...
            <div className="flex justify-between items-center bg-white   rounded-lg mb-3">
                ...
                <div>
                    ...
                    <div className="text-sm text-gray-500">
                        ...
                        &gt;
                        <Link to="/dashboard/orders" className="hover:text-primary">
                            Orders
                        </Link>{" "}
                        &gt;
                ...
                {!showFilters ? (
                    ...
                    <div className="flex justify-center items-center gap-3">
                        ...
                        <div className="relative flex-grow md:flex-grow-0 w-full md:w-auto">
                            ...
                            <input
                                type={["unitprice", "discount"].includes(filterType) ? "number" : "text"}
                ...
                <div className="bg-white  rounded-lg overflow-hidden">
                    <table className="w-full text-sm text-left text-gray-700">
                        <thead className="bg-gray-100">
                            <tr>
                                {[
                                    { key: "productName", label: "Proudct Name" },
                                    ...
                                    sortedOrders.map((order, index) => (
                                        <tr key={index} className="border-t hover:bg-gray-50">
                                            ...
                                            <td className="relative space-x-2">
                                                <FiEye onClick={() => nav(`/dashboard/products/product/${order.productID}`)} className="absolute top-[calc(50%-8px)] left-2 text-blue-500 cursor-pointer hover:scale-110 transition-transform" />
